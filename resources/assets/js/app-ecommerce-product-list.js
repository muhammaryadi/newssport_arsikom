...
$(function () {
  ...
  // datatable kategori
  var dt_product_table = $('#datatables-kategori'),
    katAdd = baseUrl + 'admin/kategori/create',
    categoryObj = {
      0: { title: 'Household' },
      1: { title: 'Office' },
      2: { title: 'Electronics' },
  ...
  if (dt_product_table.length) {
    var dt_kategori = dt_product_table.DataTable({
      ajax: baseUrl + 'api/kategori', // JSON file to add data
      columns: [
        // columns according to JSON
        { data: 'id' },
        { data: 'id' },
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
                '<div class="d-inline-block text-nowrap">' +
                '<a href="' + baseUrl + 'admin/kategori/' + full["id"] + '/edit" class="btn btn-sm btn-icon"><i class="ti ti-edit"></i></a>' +
                '<form action="' + baseUrl + 'admin/kategori/' + full['id'] + '" method="post" class="d-inline">' +
  ...
  // Delete Record
  $('.datatables-kategori tbody').on('click', '.delete-record', function () {
    dt_kategori.row($(this).parents('tr')).remove().draw();
  });
  ...
  var dt_product_table = $('#datatables-article'),
  articleAdd = baseUrl + 'admin/article/create',
  categoryObj = {
    0: { title: 'Household' },
    1: { title: 'Office' },
  }
  ...
  if (dt_product_table.length) {
    var dt_kategori = dt_product_table.DataTable({
      ajax: baseUrl + 'api/article', // JSON file to add data
      columns: [
        // columns according to JSON
        { data: 'id' },
        { data: 'id' },
      ...
      columnDefs: [
        ...
        },
        {
          ...
          responsivePriority: 5,
          render: function (data, type, full, meta) {
            return "<span class='text-truncate d-flex align-items-center'>"+ full.category.category_name +"</span>";
          }
        },
        {
          // Actions
          ...
          render: function (data, type, full, meta) {
            return (
                '<div class="d-inline-block text-nowrap">' +
                '<a href="' + baseUrl + 'admin/article/' + full["id"] + '/edit" class="btn btn-sm btn-icon"><i class="ti ti-edit"></i></a>' +
                '<form action="' + baseUrl + 'admin/article/' + full['id'] + '" method="post" class="d-inline">' +
                '<input type="hidden" name="_token" class="det-token" value="' + document.querySelector('meta[name="_token"]').getAttribute('content') + '">' +
                '<input type="hidden" name="_method" value="DELETE">' +
                '<button type="submit" class="btn btn-sm btn-icon"><i class="ti ti-trash"></i></button>' +
                '</form>' +
      ...
      buttons: [
        ...
        {
          text: '<i class="ti ti-plus me-0 me-sm-1 ti-xs"></i><span class="d-none d-sm-inline-block">Tambah Article</span>',
          className: 'add-new btn btn-primary ms-2 ms-sm-0 waves-effect waves-light',
          action: function () {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          full['order'].forEach((item, index) => {
            order += `- ${item['kategori']['nama']} (${condition[item['condition']]}, ${level[item['level']]}) <br> QTY: ${item['qty']} <br>`;
